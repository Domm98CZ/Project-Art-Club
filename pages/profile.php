...
if(!EMPTY($_SESSION["USER_ID"])) 
{
  ...
  if(!EMPTY($_GET["id"]) && is_numeric($_GET["id"]))
  {
    ...
    if(!$user_info) echo "<meta http-equiv='refresh' content='0;url=".$pInfo["P_MAIN"]."/".$pInfo["P_PROFILE"]."'>";
    if(!EMPTY($user_info))
    {
      echo "
  ...
  else 
  {
    ...
    <div class='user_info'>
    <div id='user_options'>
    <a href='".$pInfo["P_MAIN"]."/".$pInfo["P_PROFILE:ID"].$_SESSION["USER_ID"]."'>Zobrazit profil jako veřejnost</a>
    <div id='formular_user_privat'>
    ";
    
    echo "
    <form metod='post'>
      <h2>Zobrazované jméno</h2>
      <p>Aktualní:</p><input type='text' value='".$_SESSION["USER_DISPLAYNAME"]."' readonly>
      <p>Návrhy:</p>
      <select name='navrh'>
      ...
      echo "</select>
      <input type='submit' name='select_navrh' value='Použít'> 
      <p>Vlastní:</p><input type='text' name='display_name'>
      <input type='submit' name='select_dname' value='Použít'><br />
    </form>";
    ...
    <form metod='post'>
      <h2>Změna hesla</h2>
      <p>Staré:</p><input type='password' name='old_pass'>
      <p>Nové:</p><input type='password' name='new_pass1'>
      <p>Znovu:</p><input type='password' name='new_pass2'>
    ...
    <form metod='post'>
      ...
      <p>Aktuální:</p><input type='text' value='".$_SESSION["USER_MAIL"]."' readonly>
      <p>Nový:</p><input type='text' name='new_mail'>
    ...
    <form method='post' enctype='multipart/form-data'>
      <h2>Změna Avatara</h2>
      <p>Aktualní:</p><img src='".$_SESSION["USER_AVATAR"]."' width='".$pInfo["F_AVATAR:WIDTH"]."px' height='".$pInfo["F_AVATAR:HEIGHT"]."px'><br /> 
      <p>URL avatara:</p><input type='text' name='avatar_url'><br />
      <p>Nebo nahrát avatara:</p><input type='file' name='file'>
